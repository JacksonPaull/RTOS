...
int main(void){ //int main_ChristmasTree(void){
  uint32_t run, maxrun;
  // **** LED string initialization ****
  WS2812_Init();            // set system clock to 80 MHz, initialize SysTick, initialize PE3-0 and make them output
  // **** GPIO Port F initialization ****
                            // activate clock for Port F
  SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R5;
                            // allow time for clock to stabilize
  ...
  WS2812_AddColor(0, 0, 0, 2);
  WS2812_AddColor(128, 0, 0, 2);   // third LED of row 2 red
  WS2812_AddColor(0, 0, 0, 3);
  WS2812_AddColor(0, 0, 0, 3);
  WS2812_AddColor(0, 0, 0, 3);
  WS2812_AddColor(0, 0, 128, 3);   // fourth LED of row 3 blue
  WS2812_PrintBuffer();
...
/*  // wait for switch pressed
  ...
  while(1){
    ...
    while((SWITCHES == (SW1|SW2) && (run < maxrun))){ // repeat while switches released or for about 4 minutes
      Pattern_RainbowWaves(1, NUMCOLS, 0);
      // decrement 'phase' since it should only be incremented once per four calls
      if(phase == 0){
        phase = NUMCOLS - 1;
      } else{
        phase = phase - 1;
      ...
      Pattern_RainbowWaves(1, NUMCOLS, 3);
